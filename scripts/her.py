...
def get_small_rotation_gates():
    """
    Returns a list of small-angle rotation gates and a list of gate descriptions.
    These gates can be applied repeatedly to approximate any single-qubit unitary.
    """
    gate_descriptions = ["rxp", "rxn", "ryp", "ryn", "rzp", "rzn"]
    gate_matrices = [
        rotation_gate('x',  np.pi / 128),   # rx + angle
        rotation_gate('x', -np.pi / 128),   # rx - angle
        rotation_gate('y',  np.pi / 128),   # ry + angle
        rotation_gate('y', -np.pi / 128),   # ry - angle
        rotation_gate('z',  np.pi / 128),   # rz + angle
        rotation_gate('z', -np.pi / 128)    # rz - angle
    ]
    return gate_matrices, gate_descriptions
...
class QuantumCompilerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        success = fidelity >= self.accuracy
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        rewards = np.where(fidelities >= self.accuracy, 0, fidelities - 1.0)
...
if __name__ == '__main__':
    ...
    gate_matrices, gate_descriptions = get_small_rotation_gates()
    ...
    # Train the model
    total_timesteps = 100000000 # Adjust based on your computational resources
