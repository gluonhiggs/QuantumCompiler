...
gate_matrices = [
    rotation_gate('x', np.pi / 64),
    rotation_gate('x', -np.pi / 64),
    rotation_gate('y', np.pi / 64),
    rotation_gate('y', -np.pi / 64),
    rotation_gate('z', np.pi / 64),
    rotation_gate('z', -np.pi / 64)
]
...
class QuantumCompilerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if fidelity < self.tolerance:
            reward -= (1-fidelity)/self.max_steps
        if fidelity >= self.tolerance:
            reward += 0.1 * (self.max_steps - self.current_step) / self.max_steps
        if done and fidelity >= self.tolerance:
            reward += 10 * (self.max_steps - self.current_step)
