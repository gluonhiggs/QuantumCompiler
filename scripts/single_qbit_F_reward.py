...
class PlottingCallback(BaseCallback):
    ...
    def _on_training_end(self):
        ...
        plt.legend()
        if self.save_path:
            plt.savefig(os.path.join(self.save_path, "single_qbit_F_reward.png"))
        plt.close()
        ...
        plt.legend()
        if self.save_path:
            plt.savefig(os.path.join(self.save_path, "single_qbit_F_reward_length.png"))
        plt.close()
...
def evaluate_agent(model:DQN, vec_env, num_episodes=5):
    ...
    for _ in range(num_episodes):
        ...
        if diff <= env.tolerance:
            ...
            with open("gate_sequence_F_reward.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    # Save the model
    model.save("single_qbit_F_reward")
    # # Load the model
