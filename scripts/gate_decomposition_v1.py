...
# Round small numbers to make the output cleaner
def clean_matrix(matrix, precision=17):
    return np.round(matrix, decimals=precision)
...
# Clean the matrices before printing
RZ_actual_clean = clean_matrix(RZ_actual)
RZ_decomposed_matrix_clean = clean_matrix(RZ_decomposed_matrix)

# Print results
print(f"RZ matrix for theta = {theta_test}:\n{RZ_actual_clean}")
print("\nDecomposed RZ matrix:")
print(RZ_decomposed_matrix_clean)

# Check if the decomposed RZ matches the actual RZ matrix
rz_result = np.allclose(RZ_actual, RZ_decomposed_matrix)
print(f"\nDecomposed RZ matches actual RZ: {rz_result}")
