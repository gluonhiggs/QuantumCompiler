...
class QuantumCompilerEnv(gym.Env):
    ...
    def average_gate_fidelity(self, U, V):
        ...
        # fidelity = (1 / d * (d+1)) * (np.abs(trace_U_dagger_V)**2 + d)
        """
          IBM fidelity
        """
        d = 2
...
class PlottingCallback(BaseCallback):
    ...
    def _on_training_end(self) -> None:
        ...
        if self.save_path:
            plt.savefig(os.path.join(self.save_path, "training_plots_v2.png"))
        plt.close()
...
def evaluate_agent(model, env, num_episodes=1):
    ...
    for _ in range(num_episodes):
        ...
        if fidelity >= env.tolerance:
            ...
            #  Print the resultant matrix after applying the gate sequence
